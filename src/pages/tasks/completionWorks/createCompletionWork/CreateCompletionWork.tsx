...
export default function CreateCompletionWork() {
  useDocumentTitle('Adicionar Trabalho');
  const { user } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Trabalho criada com sucesso!');
      navigate('/admin');
    } catch (error) {
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.title}>Criar Novo Trabalho</h2>
      <form style={styles.form} onSubmit={handleSubmit}>
        <div style={styles.formGroup}>
          <label style={styles.label}>Nome do Trabalho:</label>
          <input
            type="text"
            value={name}
        ...
        <div style={styles.buttonGroup}>
          <button type="submit" style={styles.button}>Adicionar Trabalho</button>
